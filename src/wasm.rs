...
pub fn rect_generate_svg(
    ...
    GreekKeyRectConfig::new(size, width, height, border_margin, stroke_width)
        .map(|c| rect::generate_svg_string(&c, stroke_color, stroke_opacity))
...
pub fn circle_generate_svg(
    ...
    GreekKeyCircleConfig::new(radius, pattern_count, border_margin, stroke_width)
        .map(|c| circle::generate_svg_string(&c, stroke_color, stroke_opacity))
        .map_err(|e| JsValue::from_str(&e.to_string()))
}
...
mod tests {
    ...
    fn rect_svg_contains_svg_element() {
        let svg = rect_generate_svg(25, 16, 9, 10, 3.0, "#AB8E0E", 0.7).unwrap();
    ...
    fn rect_invalid_size_returns_error() {
        let err = rect_generate_svg(0, 16, 9, 10, 3.0, "#AB8E0E", 0.7).unwrap_err();
        assert!(err.as_string().unwrap().contains("--size"));
    }

    #[wasm_bindgen_test]
    fn rect_invalid_opacity_returns_error() {
        let err = rect_generate_svg(25, 16, 9, 10, 3.0, "#AB8E0E", 1.1).unwrap_err();
        assert!(err.as_string().unwrap().contains("--stroke-opacity"));
    }

    #[wasm_bindgen_test]
    fn circle_svg_contains_svg_element() {
        let svg = circle_generate_svg(300.0, 30, 10, 3.0, "#AB8E0E", 0.7).unwrap();
        assert!(svg.contains("<svg"));
        assert!(svg.contains("<path"));
    }
    ...
    fn circle_invalid_radius_returns_error() {
        let err = circle_generate_svg(0.0, 30, 10, 3.0, "#AB8E0E", 0.7).unwrap_err();
        assert!(err.as_string().unwrap().contains("--radius"));
    }

    #[wasm_bindgen_test]
    fn circle_nan_radius_returns_error() {
        let err = circle_generate_svg(f64::NAN, 30, 10, 3.0, "#AB8E0E", 0.7).unwrap_err();
        assert!(err.as_string().unwrap().contains("--radius"));
    }

    #[wasm_bindgen_test]
    fn circle_infinite_radius_returns_error() {
        let err = circle_generate_svg(f64::INFINITY, 30, 10, 3.0, "#AB8E0E", 0.7).unwrap_err();
        assert!(err.as_string().unwrap().contains("--radius"));
    }

    #[wasm_bindgen_test]
    fn circle_nan_opacity_returns_error() {
        let err = circle_generate_svg(300.0, 30, 10, 3.0, "#AB8E0E", f32::NAN).unwrap_err();
        assert!(err.as_string().unwrap().contains("--stroke-opacity"));
    }
}
